...
import { getExplainedReadiness } from "@/lib/api-client";
import { ExplainPanel, WhyButton } from "@/components/explain";
...
export function ReadinessGauge({
  ...
  // Fetch explained readiness when explanation is requested
  const {
    data: explainedReadiness,
    isLoading: isLoadingExplanation,
    refetch: fetchExplanation,
  } = useQuery({
    queryKey: ["explainedReadiness"],
    queryFn: () => getExplainedReadiness(),
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="text-center">
        ...
        {/* Why button */}
        <div className="mt-4">
          <WhyButton onClick={handleShowExplanation} />
        </div>
      </div>
        ...
        <div className="w-full mt-6">
          {isLoadingExplanation ? (
            <div className="flex items-center justify-center p-8">
              ...
              <span className="ml-2 text-gray-400">Loading explanation...</span>
            </div>
          ) : explainedReadiness ? (
            <ExplainPanel
              recommendation={explainedReadiness.recommendation}
              title="Why this readiness score?"
