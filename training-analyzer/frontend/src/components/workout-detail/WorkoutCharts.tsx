...
const COLORS = {
  ...
  pace: '#3b82f6',
  speed: '#22c55e',
  cadence: '#a855f7',  // Purple for cadence
  power: '#f59e0b',    // Amber for power
  elevation: '#78716c',
...
type ChartType = 'heartRate' | 'pace' | 'power' | 'cadence' | 'elevation';
...
export function WorkoutCharts({
  ...
  // Check data availability first (needed for default visible charts)
  const hasHRData = timeSeries.heart_rate.length > 0;
  const hasPaceData = timeSeries.pace_or_speed.length > 0;
  const hasPowerData = timeSeries.power && timeSeries.power.length > 0;
  const hasCadenceData = timeSeries.cadence && timeSeries.cadence.length > 0;
  const hasElevationData = timeSeries.elevation.length > 0;

  // Calculate default visible charts based on available data
  const defaultVisibleCharts = useMemo(() => {
    const charts: ChartType[] = [];
    if (hasPaceData) charts.push('pace');
    if (hasHRData) charts.push('heartRate');
    if (hasPowerData) charts.push('power');
    if (hasCadenceData) charts.push('cadence');
    if (hasElevationData) charts.push('elevation');
    return new Set(charts);
  }, [hasHRData, hasPaceData, hasPowerData, hasCadenceData, hasElevationData]);

  // Track which charts are visible (dynamically default to all available)
  const [visibleCharts, setVisibleCharts] = useState<Set<ChartType>>(defaultVisibleCharts);
  ...
  // Available charts configuration - ordered: Pace, HR, Power, Cadence, Elevation
  const availableCharts: ChartConfig[] = useMemo(() => {
    ...
    // Power third (only if data available)
    if (hasPowerData) {
      charts.push({
        id: 'power',
        label: 'Power',
    ...
    return charts;
  }, [hasHRData, hasPaceData, hasPowerData, hasCadenceData, hasElevationData, isRunning]);

  const toggleChart = useCallback((chartId: ChartType) => {
    setVisibleCharts(prev => {
      const newSet = new Set(prev);
      if (newSet.has(chartId)) {
        // Don't allow hiding all charts
        if (newSet.size > 1) {
  ...
  if (!hasHRData && !hasPaceData && !hasPowerData && !hasCadenceData && !hasElevationData) {
    return (
            ...
            <button
              ...
              className={`
                ...
                ${visibleCharts.has(chart.id)
                  ? `${chart.color} text-white shadow-sm`
      ...
      <div className="h-40 sm:h-48">
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            />
            <Tooltip
              content={<CustomTooltip metricName="HR" unit=" bpm" color="text-red-400" />}
...
function PaceSpeedChart({ data, isRunning, syncedHover, onMouseMove, onMouseLeave }: PaceSpeedChartProps) {
  ...
  const tooltipColor = isRunning ? 'text-blue-400' : 'text-green-400';
...
function CadenceChart({ data, isRunning, syncedHover, onMouseMove, onMouseLeave }: CadenceChartProps) {
  const chartData = useMemo(() => {
    const downsampled = downsampleData(data, 400);
    return downsampled.map(point => ({
      timestamp: point.timestamp,
  ...
  const { minCadence, maxCadence, avgCadence } = useMemo(() => {
    const cadences = chartData.map(d => d.cadence).filter(c => c > 0);
    if (cadences.length === 0) return { minCadence: 0, maxCadence: 200, avgCadence: 0 };

    const min = Math.min(...cadences);
    const max = Math.max(...cadences);
    const avg = Math.round(cadences.reduce((a, b) => a + b, 0) / cadences.length);
  ...
  const syncedData = useMemo(() => {
    if (syncedHover.activeIndex === null || syncedHover.activeIndex >= chartData.length) return null;
    const point = chartData[syncedHover.activeIndex];
    return { timestamp: point.timestamp, value: point.cadence };
  }, [syncedHover.activeIndex, chartData]);

  // Optimal cadence reference zones
  const optimalMin = isRunning ? 170 : 85;
  const optimalMax = isRunning ? 185 : 95;
        ...
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <YAxis
              ...
              tickLine={false}
              axisLine={false}
              domain={[minCadence, maxCadence]}
              width={40}
              tickFormatter={(val) => `${val}`}
            />
            <Tooltip
              content={<CustomTooltip metricName={title} unit={` ${unit}`} color="text-purple-400" />}
              cursor={{ stroke: '#6b7280', strokeDasharray: '3 3' }}
            />
            {/* Optimal cadence zone reference lines */}
            <ReferenceLine
              y={optimalMin}
              stroke="#22c55e"
              strokeDasharray="5 5"
              strokeOpacity={0.5}
              label={{ value: `${optimalMin}`, position: 'left', fill: '#22c55e', fontSize: 10 }}
            />
            <ReferenceLine
              y={optimalMax}
              stroke="#22c55e"
              strokeDasharray="5 5"
              strokeOpacity={0.5}
              label={{ value: `${optimalMax}`, position: 'left', fill: '#22c55e', fontSize: 10 }}
            />
            {/* Synced vertical reference line */}
            {syncedData && (
              <ReferenceLine
...
function PowerChart({ data, syncedHover, onMouseMove, onMouseLeave }: PowerChartProps) {
  const chartData = useMemo(() => {
    const downsampled = downsampleData(data, 400);
    return downsampled.map(point => ({
      timestamp: point.timestamp,
  ...
  const { minPower, maxPower, avgPower } = useMemo(() => {
    const powers = chartData.map(d => d.power).filter(p => p > 0);
    if (powers.length === 0) return { minPower: 0, maxPower: 500, avgPower: 0 };
    ...
    return {
      minPower: Math.max(0, min - padding),
      maxPower: max + padding,
      avgPower: avg,
    };
  }, [chartData]);
      ...
      <div className="h-40 sm:h-48">
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            />
            <Tooltip
              content={<CustomTooltip metricName="Power" unit="W" color="text-amber-400" />}
              cursor={{ stroke: '#6b7280', strokeDasharray: '3 3' }}
            />
      ...
      <div className="h-28 sm:h-36">
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            />
            <Tooltip
              content={<CustomTooltip metricName="Elevation" unit="m" valueFormatter={(v: number) => v.toFixed(0)} color="text-stone-400" />}
              cursor={{ stroke: '#6b7280', strokeDasharray: '3 3' }}
            />
            {/* Synced vertical reference line */}
            {syncedData && (
...
interface CustomTooltipProps {
  ...
  label?: string | number;  // Recharts passes this automatically (x-axis value)
  metricName: string;       // Our custom label (e.g., "HR", "Pace")
  unit: string;
  valueFormatter?: (value: number) => string;
  color?: string;           // Color for the value
}

function CustomTooltip({ active, payload, metricName, unit, valueFormatter = (v) => v.toString(), color = 'text-teal-400' }: CustomTooltipProps) {
